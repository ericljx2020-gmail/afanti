...
注意事项：
1. 对于每一题都当作是新题来讲解，不要出现任何类似于“根据我们刚才说的”这样的话，每一个讲解必须以答案结尾，不能够没有答案。
2. 对于生成的相似题，不能只是简单的替换数字或者题目意思，需要保证和原题有较大区别，但是解题思路和讲解思路可以相互借鉴。
"""
...
1. 对于每一题都当作是新题来讲解，不要出现任何类似于“根据我们刚才说的”这样的话，每一个讲解必须以答案结尾，不能够没有答案。
2. 对于生成的相似题，不能只是简单的替换数字或者题目意思，需要保证和原题有较大区别，但是解题思路和讲解思路可以相互借鉴。
3. 提取老师讲解的时候请过滤掉无用信息，直接从讲解开始的地方开始提取
"""
...
template_note = r"""
    ...
    Goals:
    根据给定信息按照要求像老师一样讲解一个题目，但不要输出任何讲解内容，只输出知识点和板书，并能够根据validator的修改建议调整板书内容。
    ...
    1. 给出该题目涉及的知识点
    2. 判断给出的信息中是不是包含原题并且有原题的解答过程，如果有原题，板书使用原题的解答步骤
    3. 根据validator的反馈调整板书内容，以确保准确性和清晰度
